...
import { addDays, daysBetween, todayDateString } from './calendar-date.util';
import { PeriodPredictionReliability } from './entities/calendar.enums';
import { PeriodPredictionDto, PeriodRangeDto } from './dtos/response.dto';
...
export class MenstrualCyclePredictionService {
  predict(ranges: PeriodRangeDto[]): PeriodPredictionDto {
    ...
    const expectedPeriodEndDate = addDays(
      expectedPeriodStartDate,
      periodDurationDays - 1,
    );
    const predictionUncertaintyDays = this.predictionUncertaintyDays(
      reliability,
      cycleModel,
    );
    ...
    return {
      ...
      cycleLengthVariabilityDays:
        cycleModel.sampleCount > 1 ? cycleModel.variabilityDays : null,
  ...
  private estimateCycleModel(cycleLengths: number[]): CycleModelEstimate {
    ...
    const regression =
      cycleLengths.length >= 3
        ? this.tensorWeightedLinearRegression(cycleLengths)
        : null;
    ...
    const blendedEstimate = stableRegressionPrediction !== null
      ? weightedAverage * (1 - regressionWeight) +
        stableRegressionPrediction * regressionWeight
      : weightedAverage;
    ...
    const variabilityDays = this.round(
      cycleLengths.length > 1
        ? this.tensorWeightedStandardDeviation(cycleLengths)
        : 0,
    );
    const modelDisagreementDays = stableRegressionPrediction !== null
    ...
    return {
      ...
      recentIrregularity:
        variabilityDays > 7 ||
        Math.abs(recentCycleLength - cycleLengthDays) > 7,
      regression,
  ...
  private reliability(model: CycleModelEstimate): PeriodPredictionReliability {
    ...
    if (
      model.sampleCount === 1 ||
      model.variabilityDays > 7 ||
      model.modelDisagreementDays > 5
    ) {
      return PeriodPredictionReliability.Low;
    }
    if (
      model.sampleCount >= 4 &&
      model.variabilityDays <= 3 &&
      model.modelDisagreementDays <= 2
    ) {
  ...
  private predictionUncertaintyDays(
    ...
    const uncertainty = Math.ceil(
      model.variabilityDays * 0.6 + model.modelDisagreementDays * 0.4,
    );
