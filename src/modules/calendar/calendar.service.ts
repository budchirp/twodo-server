...
import { isUserProfileCompleted } from '../users/user-profile.util';
...
export class CalendarService {
  ...
  async createEntry(
    ...
    const entryId = await this.dataSource.transaction(async (manager) => {
      ...
      if (
        body.type === CalendarActivityType.SexualActivity &&
        body.sexualActivity
      ) {
        await manager.save(
  ...
  async listEntries(
    ...
    return entries.map((entry) => CalendarMapper.toCalendarEntryResponse(entry));
  }
  ...
  private async currentMembership(user: User): Promise<CoupleMember> {
    ...
    if (!membership) {
      throw new ApiException(
        'error.couple_membership_required',
  ...
  private assertCanManagePeriod(user: User): void {
    if (user.gender !== UserGender.Female) {
      throw new ApiException(
        'error.period_management_forbidden',
        HttpStatus.FORBIDDEN,
      );
    }
  }

  private async entryForCouple(
    id: string,
    coupleId: string,
  ...
  private async assertPeriodEndHasStart(
    ...
    const hasStart = entries.some(
      (entry) =>
        ...
        entry.periodDetail?.event === CalendarPeriodEvent.Start,
    );
  ...
  private validateUpdatePayload(
    ...
    if (nextType === CalendarActivityType.SexualActivity) {
      const hasSexualActivityDetail =
        !!body.sexualActivity || !!entry.sexualActivityDetail;
      ...
      if (body.sexualActivity) {
        this.validateSexualActivityPayload(
          this.mergeSexualActivityDetail(entry, body),
        );
      }
      return;
    }
  ...
  private validateSexualActivityPayload(
    detail: Partial<CalendarSexualActivityDetail>,
  ): void {
    const sexOccurred = detail.sexOccurred ?? false;
    const protectionMethod =
      detail.protectionMethod ?? CalendarProtectionMethod.None;
    const ejaculationLocation =
      detail.ejaculationLocation ?? CalendarEjaculationLocation.None;
  ...
  private validateDateRange(startDate: string, endDate: string): void {
    ...
    if (startDate > endDate) {
      throw new ApiException(
        'error.invalid_calendar_date_range',
  ...
  private periodEntryForDate(
    ...
    return (
      sameDateEntries.find((entry) => entry.id === preferredEntryId) ??
      sameDateEntries[0] ??
      null
    );
  }

  private periodEventForDateIndex(
    index: number,
    length: number,
  ): CalendarPeriodEvent {
    if (index === 0) {
  ...
  private async saveGeneratedPeriodDetail(
    ...
    const detail = entry.periodDetail ?? manager.create(CalendarPeriodDetail, {
      entryId: entry.id,
    });
  ...
  private async saveSexualActivityDetail(
    ...
    const merged = this.createSexualActivityDetail(
      this.mergeSexualActivityDetail(entry, body),
    );
    const detail = entry.sexualActivityDetail ?? manager.create(
      CalendarSexualActivityDetail,
      { entryId: entry.id },
    );
