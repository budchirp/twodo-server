...
import { daysBetween } from './calendar-date.util';
import { CycleHistoryDto, PeriodRangeDto } from './dtos/response.dto';
  ...
  cycleHistory(ranges: PeriodRangeDto[]): CycleHistoryDto[] {
    return ranges.map((range, index) => ({
      ...
      cycleLengthDays:
        index === 0
          ? null
          : daysBetween(ranges[index - 1].startDate, range.startDate),
    }));
  }
